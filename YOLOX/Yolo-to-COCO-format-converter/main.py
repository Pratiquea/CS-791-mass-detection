...
def get_images_info_and_annotations(opt):
    ...
    for file_path in file_paths:
        ...
        print("label_file_name = {}".format(label_file_name))
        # print(" = {}".format())
        if opt.yolo_subdir:
            annotations_path = file_path.parent / YOLO_DARKNET_SUB_DIR / label_file_name
        else:
            annotations_path = os.path.join(opt.label_path, label_file_name)

        print("annotation path = {}".format(annotations_path))
...
def get_args():
    ...
    parser.add_argument(
        ...
        '--classes',
        type=str, 
        help='path to config file containing class names',
        required=True)
    parser.add_argument(
        '-l',
        '--label_path',
        type=str, 
        help='path to label directory containing annotations',
        required=True)
    parser.add_argument(
        "-p",
        "--path",
