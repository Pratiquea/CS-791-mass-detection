...
# python draw_box.py -i /home/rwl/gits/yolov3_quantization_aware/dataset/train/images/ -l /home/rwl/gits/yolov3_quantization_aware/dataset/train/labels/ -c /home/rwl/gits/yolov3_quantization_aware/data/dota.names
...
def draw_box_on_image(image_name, classes, colors, label_folder, raw_images_folder, save_images_folder ):
    ...
    if image_name == '.DS_Store':
        return 0
    image_path = os.path.join( raw_images_folder,'%s.png'%(image_name))  #本次原始图片jpg路径
    
    save_file_path = os.path.join(save_images_folder,'%s.png'%(image_name)) #本次保存图片jpg路径
...
if __name__ == '__main__':           # 只有在文件作为脚本文件直接执行时才执行下面代码  
    ...
    parser = argparse.ArgumentParser(description="draw b box on image with yolo label annotations")
    parser.add_argument('-i','--raw_images_folder', type=str, default='./raw_images/', help='raw_images_folder')
    parser.add_argument('-l','--label_folder', type=str, default='./labels/', help='label_folder')
    parser.add_argument('-c','--classes', type=str, default='./classes.txt', help='classes')
