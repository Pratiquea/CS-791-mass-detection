...
class PreProcessing:
    ...
    def run(self):
                ...
                for row in csv_reader:
                    ...
                    if folder_name in self.mass_folder_test or folder_name in self.mass_folder_train or folder_name in self.mass_folder_val:
                        ...
                        if self.plot:
                            ...
                            if laterality == "R":
                                img_orig_flip = cv2.rectangle(img_orig_flip.astype('uint8'), (int(bbox_orig_flip[0]),int(bbox_orig_flip[1])), ( int(bbox_orig_flip[2]+bbox_orig_flip[0]),int(bbox_orig_flip[3]+bbox_orig_flip[1]) ), (255,0,0), 10)
                                ax3.imshow(img_orig_flip, cmap='gray')
                            else:
                                img_orig_csv = cv2.rectangle(img_orig_copy, (int(bbox_csv_format[0]),int(bbox_csv_format[1])), (int(bbox_csv_format[2]) ,int(bbox_csv_format[3]) ), (255,0,0), 10)
        ...
        with open("test.json", "w") as outfile3:
...
def main():
    ...
    parser.add_argument("-o","--output_path", type=str, default="/home/rwl/Downloads/Dataset/physionet.org/files/vindr-mammo/1.0.0/processed_imgs/", help="Path to output directory")
